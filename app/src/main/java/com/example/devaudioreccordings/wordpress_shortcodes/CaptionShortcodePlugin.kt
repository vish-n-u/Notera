package com.example.devaudioreccordings.wordpress_shortcodes

import android.text.Editable
import android.text.Layout
import android.text.Spannable
import android.text.SpannableStringBuilder
import com.example.devaudioreccordings.wordpress_shortcodes.handlers.CaptionHandler
import com.example.devaudioreccordings.wordpress_shortcodes.spans.CaptionShortcodeSpan
import com.example.devaudioreccordings.wordpress_shortcodes.spans.createCaptionShortcodeSpan
import com.example.devaudioreccordings.wordpress_shortcodes.watchers.CaptionWatcher
...
class CaptionShortcodePlugin @JvmOverloads constructor(private val aztecText: AztecText? = null) :
    ...
    override fun beforeSpansProcessed(spannable: SpannableStringBuilder) {
        spannable.getSpans(0, spannable.length, CaptionShortcodeSpan::class.java).forEach {
            it.attributes.removeAttribute(ALIGN_ATTRIBUTE)
            if (it is IAztecAlignmentSpan && it.align != null) {
                it.attributes.setValue(
                    ALIGN_ATTRIBUTE,
                    when (it.align) {
                        Layout.Alignment.ALIGN_NORMAL -> ALIGN_LEFT_ATTRIBUTE_VALUE
                        Layout.Alignment.ALIGN_CENTER -> ALIGN_CENTER_ATTRIBUTE_VALUE
                        else -> ALIGN_RIGHT_ATTRIBUTE_VALUE
                    })
            }
        }
    }
